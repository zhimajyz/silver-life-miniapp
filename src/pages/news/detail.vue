<template>
	<view class="container">
		<view class="card" v-if="newsDetail.id">
			<text class="title">{{ newsDetail.title }}</text>
			<view class="meta">
				<text>{{ newsDetail.source }}</text>
				<text>{{ formatTime(newsDetail.publishTime) }}</text>
			</view>
			...
			<view class="content">
				<text class="paragraph">{{ newsDetail.content }}</text>
			</view>
...
onLoad(async (options) => {
	const id = options.id;
	try {
		const data = await request({ url: `/news/detail/${id}` });
		...
		await request({
			...
			method: 'POST',
			data: { openId, itemId: id, itemType: 'news', title: data.title }
		});

		// 检查是否已收藏
		const checkRes = await request({
			url: '/favorite/check',
			data: { openId, itemId: id, itemType: 'news' }
		});
...
const fetchPrevNext = async (currentId) => {
	try {
		const res = await request({ url: '/news/list', data: { page: 1, pageSize: 100 } });
...
const goPrev = () => {
	if (prevId.value) {
		stopVoice();
		uni.redirectTo({ url: `/pages/news/detail?id=${prevId.value}` });
	}
};
...
const goNext = () => {
	if (nextId.value) {
		stopVoice();
		uni.redirectTo({ url: `/pages/news/detail?id=${nextId.value}` });
	}
};

const toggleFavorite = async () => {
	const itemId = newsDetail.value.id;
	const itemType = 'news';
	const title = newsDetail.value.title;
	const cover = newsDetail.value.cover || '';
...
const togglePlay = async () => {
	...
	try {
		...
		const plainText = newsDetail.value.content.replace(/<[^>]+>/g, '');
		...
		const filePath = `${wx.env.USER_DATA_PATH}/tts_news_${Date.now()}.mp3`;
		fs.writeFile({
			filePath,
			data: res.data,
			success: () => {
...
// 分享
onShareAppMessage(() => ({
	title: newsDetail.value.title || '银发智慧生活 - 时政新闻',
